...
def pagina_principal(request):
    ...
    try:
        if user.is_superuser or agente:
            ultimos_tickets = Tickets.objects.all().order_by('-creado')[:5]
        else:
            ultimos_tickets = Tickets.objects.filter(usuario=user).order_by('-creado')[:5]
    except:
        ultimos_tickets = []
    ...
    try:
        ...
        tickets_abiertos = Tickets.objects.filter(cerrado=False).count()
        tickets_cerrados = Tickets.objects.filter(cerrado=True).count()
...
def ver_tikects(request):
    tikects = Tickets.objects.all().order_by('-creado')
    reasignaciones_dict = {}
    try:
...
def ver_tikects_cerrados(request):
    tikects = Tickets.objects.filter(cerrado=True).order_by('-creado')
    reasignaciones_dict = {}
    try:
        for reasignacion in ReasignacionTikects.objects.all():
            if reasignacion.agente_nuevo and reasignacion.agente_nuevo.usuario:
...
def ver_tikects_abiertos(request):
    tikects = Tickets.objects.filter(cerrado=False).order_by('-creado')
    reasignaciones_dict = {}
    try:
        for reasignacion in ReasignacionTikects.objects.all():
            if reasignacion.agente_nuevo and reasignacion.agente_nuevo.usuario:
...
def ver_tikects_asignados_agentes(request):
    ...
    tikects_servicios = Tickets.objects.filter(
        servicio_perteneciente__in=[a.servicio for a in asignaciones_servicios]
    ).order_by('-creado')
    
    # Combinar todas las listas
    tikects_list = list(tikects_directos) + list(tikects_reasignados) + list(tikects_servicios)
    tikects_list.sort(key=lambda x: x.creado, reverse=True)
...
def usuarios_clientes_grupos_crear(request):
    if request.method == 'GET':
        ...
        return redirect('usuarios_clientes_grupos_crear')
...
def check_notifications(request):
    if request.user.is_authenticated:
        ...
        if agente:
            ...
            return JsonResponse({
                'new_notifications': nuevas_notificaciones.exists(), 
            ...
            for index, row in df.iterrows():
                try:
                    ...
                    ticket = Tickets(
                        ...
                        creado=creado_fecha,
                        cerrado=(str(row['Estado']).lower() == 'cerrado'),
